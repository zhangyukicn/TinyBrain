...
export default function EditImage (info) {
    ...
    const blankImg = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSi8v42_qIIylFc_HQITxDN8AQAHnoFvxKnqg&usqp=CAU';
    const id = localStorage.getItem('quiz_id');
    const questionId = localStorage.getItem('questionId');
    const questionIndex = localStorage.getItem('questionIndex');
    const token = localStorage.getItem('token');
    const history = useHistory();
    let tmpThumb = blankImg;
    if (info.info === 0) {
        tmpThumb = blankImg;
    } else {
        tmpThumb = info.info.questions[questionIndex].img;
    }
    ...
    const changeImg = () => {
        ...
        info.info.questions[questionIndex].img = thumbnail;
    ...
    return (
        ...
        <CardMedia
            ...
            image={thumbnail || tmpThumb || blankImg}
            id='imgPreview'
        />
        <CardActions>
